...
%              ImmediateReward=Reward(1,1)*ResultListRelevance(focus);
       ...
       pos = mod(n_steps,3);
       last_three_focused(1,pos+1) = focus;
     
     %mark the position of last three focused items 1
     for x = 1: 3
         has_visited = last_three_focused(1,x);
         if has_visited
...
%         available_query_words(opened) = query_words_read(opened);
    ...
    if first_run == 1
        if mod(trial,5e5)==0
            filename = strcat('Data_1min_v3_Memory_',...
        ...
        if mod(trial,ntrials)==0
            filename = strcat('Test_Data_1min_v3_Memory_',...
                                ranking_type,'_',num2str(trial));
            save([filename '.mat'],...
